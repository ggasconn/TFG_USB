...
void pwmSetup (void)
{
    TCCR0A |= 1<<WGM00; //Modo 1 Phase correct
    ...
    //Output Mode Select
    TCCR0A |= 1<<COM0A1 | 1<<COM0A0; //Salida 0C0A HIGH en el match de comparacion ascendente y LOW en el match decendente
    TCCR0A |= 1<<COM0B1 | 1<<COM0B0; //Salida 0C0B HIGH en el match de comparacion ascendente y LOW en el match decendente
     
    DDRB |= 1<<PB1 | 1<<PB0; //Eneable de salidas del PWM 
 
    TCCR0B |= 1<<CS01 | 1<<CS00; //Clock preescalado entre 64 e inicio del Timer/Counter 0
} 
